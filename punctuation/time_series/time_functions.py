...
#                                      ,list_max, list_min):
    ...
    #      elif method == 'z':
        ...
        ax.plot(list_dates,list_mean, color=color_vector[i],
                label=(options.punctuation_vector[:-1]+['...'])[int(col[-1])], 
                marker=marker_vector[i],#linestyle='None',
                )
        if show_ci:
    ...
    if print_legend:
        plt.legend(bbox_to_anchor=(0., 1.02, 1, .102),
                   fontsize=16, loc=3, labelspacing=1, handletextpad=0.5,
                   handleheight=2, markerscale=1.8, columnspacing=0.0005,
                   mode="expand", ncol=5, frameon=False)
    if to_show:
        plt.show()
...
#                         label='Color Patch')]
#
#plt.legend(handles=legend_elements, bbox_to_anchor=(0., 1.02, 1, .102),
#                   fontsize=15, loc=3, #labelspacing=0.05,
#                   handleheight=2, markerscale=1, columnspacing=0.0005,
#                   mode="expand", ncol=5, frameon=False)
#
