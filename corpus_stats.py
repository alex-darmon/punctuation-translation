...
def plot_features(df):
    ...
    plot_trans_mat(np.reshape(np.array(df[options.mat_nb_words_pun_col].mean()), (10,10)),
                       punctuation_vector=options.punctuation_vector)
    plot_trans_mat(np.reshape(np.array(df[options.transition_mat_col].mean()), (10,10)),
                       punctuation_vector=options.punctuation_vector)
    plot_trans_mat(np.reshape(np.array(df[options.norm_transition_mat_col].mean()), (10,10)),
                       punctuation_vector=options.punctuation_vector)
...
df_text_final = df_text_final[df_text_final.columns.tolist()[:-7]]
df_text_final = pd.merge(df_text_final, df_cache, on='book_id')

for lang in df_text_final['language'].drop_duplicates().tolist():
    print(lang)
    df_l = df_text_final[df_text_final['language']==lang]
    plot_features(df_l)
    
    
df_freq_lang = df_text_final.groupby('language')[options.freq_pun_col].mean()
df_freq_lang.columns = options.punctuation_vector
print(tabulate(df_freq_lang, headers='keys'))